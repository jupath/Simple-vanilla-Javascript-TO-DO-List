const todoApp = (function () {
  ...
  // Populate html
  function populateTodo(populateTodos = [], populateTodoList) {
    populateTodoList.innerHTML = populateTodos.map(todo => `
  ...
  function updateItemsLeft() {
    const numberOfTodos = todos.filter(todo => todo.done === false).length;
  ...
  function addTodo(event) {
    ...
    // Create an ID for the new item. If the todo array is empty ID is 0,
    // if not find the biggest number and add 1
  ...
  function handleTodo(event) {
    ...
    if (event.target.nodeName === 'INPUT') {
      ...
      const obj = todos.find(cur => cur.ID === id);
